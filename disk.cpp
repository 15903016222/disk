...
char *readSectorDataFromHandle(QuaZipFile * handle, unsigned long long startsector, unsigned long long numsectors, unsigned long long sectorsize)
{
    unsigned long bytesread;
    char *data = new char[sectorsize * numsectors];
    if(!(bytesread = handle->read(data,sectorsize * numsectors)))
    {
        qDebug()<<"read error";
        delete[] data;
        data = NULL;
...
char *readSectorDataFromHandle1(HANDLE handle, unsigned long long startsector, unsigned long long numsectors, unsigned long long sectorsize)
{
    unsigned long bytesread;
    char *data = new char[sectorsize * numsectors];
    LARGE_INTEGER li;
